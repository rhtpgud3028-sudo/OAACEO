...
// v20.9 TTS-SYNC-PERFECT (2026-01-16)
// ============================================================
// 🔥 v20.9 핵심 변경:
// 1. ElevenLabs with-timestamps API 연동 (동적 싱크!)
// 2. 문장별 실제 TTS 발화 시간 기반 자막/슬라이드 배치
// 3. 65초 영상 완벽 지원 (TTS 65초 + 동적 싱크)
// 4. 물흐르듯 자연스러운 TTS-자막-슬라이드 싱크!
// ============================================================
...
try {
    // 🆕 v20.9: ElevenLabs with-timestamps 응답에서 alignment 가져오기
    const elevenLabsNode = $('ElevenLabs TTS').first();
    ...
    if (elevenLabsNode?.json?.audio_base64) {
        // Upload Audio 노드에서 저장된 URL 가져오기
...
if (!ttsAudioUrl) {
    try {
        ...
        // 추가 폴백: Upload Audio 노드 (HTTP Request 방식)
        if (!ttsAudioUrl) {
            const uploadResponse = $('Upload Audio').first()?.json;
            if (uploadResponse) {
                if (uploadResponse.url) {
