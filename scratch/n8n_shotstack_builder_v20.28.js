...
// v20.28 (v19.4 자막 규칙 복원!)
// ============================================================
// 🔴 핵심: v19.4 자막 규칙 적용!
// - Width: 1000px (v20.12의 850px가 잘림 원인!)
// - Offset Y: -0.23
...
const SUBTITLE_V19_RULES = {
    ...
    style: {
        lineHeight: 1.6,           // v19.4 그대로!
        letterSpacing: 2
    },
    background: {
        color: brandingConfig?.branding?.primaryColor || "#000000",
        opacity: 0.6              // 🔴 v19.4: 0.6! (v20.27은 0.75였음)
    },
    size: {
        width: 1000,              // 🔴 v19.4: 1000px! (v20.27은 850px였음)
        height: 400
    },
    position: {
        offsetX: 0,
        offsetY: -0.23            // 🔴 v19.4: -0.23! (v20.27은 -0.25였음)
    }
};
...
function formatSubtitle(text) {
    ...
    const MAX_CHARS_PER_LINE = 14;  // v19.4: 14자/줄
    const lines = [];
...
if (hookText) {
    ...
    const hookHeight = Math.max(200, hookLineCount * 100 + 50);  // v19.4 동적 높이
...
for (let i = 0; i < segments.length; i++) {
    ...
    const dynamicHeight = Math.max(200, lineCount * 100 + 50);  // v19.4: 동적 높이!

    subtitleClips.push({
        asset: {
            type: "rich-text",
            text: formattedText,
            font: SUBTITLE_V19_RULES.font,
            style: SUBTITLE_V19_RULES.style,
            background: SUBTITLE_V19_RULES.background,
            align: { horizontal: "center", vertical: "middle" }
        },
        start: subtitleStartTimes[i],
        length: segmentTimes[i],
        position: "center",
        width: SUBTITLE_V19_RULES.size.width,    // 🔴 1000px!
        height: dynamicHeight,
        offset: {
            x: SUBTITLE_V19_RULES.position.offsetX,
            y: SUBTITLE_V19_RULES.position.offsetY  // 🔴 -0.23!
        }
    });
}
...
return [{
    json: {
        ...
        debug: {
            version: "20.28-V19.4-RESTORED",
            note: "v19.4 자막 규칙 복원! Width=1000, OffsetY=-0.23",
            v19Rules: {
                width: SUBTITLE_V19_RULES.size.width,
                offsetY: SUBTITLE_V19_RULES.position.offsetY,
                backgroundOpacity: SUBTITLE_V19_RULES.background.opacity,
                fontSize: SUBTITLE_V19_RULES.font.size
            },
            timing: {
