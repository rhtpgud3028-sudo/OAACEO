...
let ttsAudioUrl = "";
try {
    // 🔴 Convert Audio 노드에서 최종 .mp3 URL 가져오기 (Save Audio는 .mpga!)
    ttsAudioUrl = $('Convert Audio').first()?.json?.url || "";
    // 폴백: Convert Audio가 없으면 Boost Body Audio에서 시도
    if (!ttsAudioUrl) {
        ttsAudioUrl = $('Boost Body Audio').first()?.json?.url || "";
    }
} catch (e) { }
