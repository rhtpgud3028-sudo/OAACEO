...
// v20.7 HOOK-TTS-FIX + 0.5s SYNC (2026-01-14)
// ============================================================
// 🔥 v20.7 핵심 변경:
// 1. hookTtsUrl 경로 수정 (Upload Hook Audio 노드 → JSON.parse 적용)
// 2. 자막 & 슬라이드 0.5초 앞당김 (TTS 싱크 최적화!)
// 3. 0-3초: 훅 TTS + 훅 자막 + 인트로 영상
// 4. 3-5초: 인트로 타이틀 + 인트로 영상
// 5. 5-50초: 본문 TTS + 자막 + 슬라이드 (0.5초 앞당김!)
// ============================================================
...
// 🆕 v20.7: TTS 싱크 오프셋 (자막 & 슬라이드를 TTS보다 앞당김)
const SYNC_OFFSET = 0.5;  // 0.5초 앞당김!
...
// 🆕 v20.7: 훅 TTS URL (Upload Hook Audio HTTP Request 노드에서!)
let hookTtsUrl = "";
try {
    // HTTP Request 노드는 JSON 응답을 data 필드에 문자열로 반환
    const uploadResponse = $('Upload Hook Audio').first()?.json;
    if (uploadResponse) {
        // 직접 url 필드가 있으면 사용
        if (uploadResponse.url) {
            hookTtsUrl = uploadResponse.url;
        }
        // data 필드에 JSON 문자열이 있으면 파싱
        else if (uploadResponse.data) {
            const parsed = safeParseJSON(uploadResponse.data, {});
...
// 🆕 v20.7.1: 본문 TTS URL (Save Audio = Write Binary File 노드!)
let ttsAudioUrl = "";
try {
    const saveAudioNode = $('Save Audio').first();
    if (saveAudioNode) {
        // Write Binary File 노드 출력 형식들:
        // 1. json.fileName (가장 일반적)
        // 2. json.file (파일 객체)
        // 3. binary 데이터에서 추출
        let filePath = "";
        ...
        if (filePath) {
            // /var/www/html/audio/xxx.mp3 → xxx.mp3
    ...
    // 폴백: Upload Audio 노드 (HTTP Request 방식)
    if (!ttsAudioUrl) {
        const uploadResponse = $('Upload Audio').first()?.json;
        if (uploadResponse) {
            if (uploadResponse.url) {
                ttsAudioUrl = uploadResponse.url;
            } else if (uploadResponse.data) {
                const parsed = safeParseJSON(uploadResponse.data, {});
...
// ==================== 4) 시간 배분 (v20.7: 0.5초 앞당김!) ====================
const SUBTITLE_START = BODY_START - SYNC_OFFSET;  // 4.5초부터 본문 자막 시작!
const IMAGE_START = BODY_START - SYNC_OFFSET;      // 4.5초부터 슬라이드 시작!
...
// 🆕 v20.7: 자막용 시작 시간 (0.5초 앞당김!)
const subtitleStartTimes = [];
...
// ==================== 6) 자막 클립 (v20.7: 훅 자막 + 0.5초 싱크!) ====================
const subtitleClips = [];

// 🆕 v20.7: 훅 자막 클립 (0-3초) - 훅 자막도 0.5초 앞당기지 않음 (인트로와 동시 시작)
if (hookText) {
...
for (let i = 0; i < segments.length; i++) {
    ...
    subtitleClips.push({
        ...
        start: subtitleStartTimes[i],  // 🆕 0.5초 앞당긴 시작 시간!
...
// 🆕 v20.7: 본문 슬라이드 (0.5초 앞당김!)
for (let i = 0; i < segments.length; i++) {
    const src = images[i] || images[images.length - 1] || placeholderImage;
    const slideClip = {
        asset: { type: "image", src },
        start: imageStartTimes[i],  // 🆕 0.5초 앞당긴 시작 시간!
        length: segmentTimes[i] + 0.5,  // 🆕 길이도 0.5초 연장 (겹침 방지)
        fit: "cover"
    };
    if (!DISABLE_TRANSITIONS) {
...
// 본문 TTS (5초~) - TTS 시작 시간은 변경 없음!
if (ttsAudioUrl) {
    audioClips.push({
        asset: { type: "audio", src: ttsAudioUrl },
        start: BODY_START,  // 5초부터 (TTS는 그대로!)
        length: BODY_LEN    // 45초
    });
}
...
return [{
    json: {
        ...
        debug: {
            version: "20.7-HOOK-TTS-FIX-SYNC",
