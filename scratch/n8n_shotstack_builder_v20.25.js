...
// v20.25 MINIMAL-FIX (v20.22 기반)
// - v20.22 그대로 + 딱 3가지만 수정:
// - 1) 폰트: 45px → 60px (5060세대)
// - 2) offset.y: 0.07 → 0.20 (YouTube UI 피함)
// - 3) TTS 기반 동적 영상 길이 (검은화면 방지)
// ============================================================
...
// ✅ v20.25: offset.y = 0.20 (20% = 384px 위로) - YouTube UI 피함!
const SAFE_BOTTOM_Y = 0.20;

// ✅ v20.25: 5060세대 가독성 = 60px 폰트!
const SUBTITLE_FONT_SIZE = 60;   // 🔴 45 → 60 (5060세대!)
const SUBTITLE_PADDING = 24;     // 22 → 24 (여백 확보)
...
let ttsActualEndTime = 0;  // ✅ v20.25: TTS 실제 끝 시간
...
// ✅ v20.25: TTS 기반 동적 영상 길이 (검은화면 방지!)
...
if (USE_DYNAMIC_SYNC && ttsAlignment) {
    ...
    for (let i = 0; i < segments.length; i++) {
        ...
        if (i === segments.length - 1) {
            const remainingTime = ACTUAL_VIDEO_LENGTH - startTime;  // ✅ 동적!
    ...
    if (segmentTimes.length > 0) {
        ...
        const remaining = ACTUAL_VIDEO_LENGTH - lastStart;  // ✅ 동적!
...
for (let i = 0; i < segments.length; i++) {
    ...
    let slideLength;
    if (i === segments.length - 1) {
        const calculatedLength = ACTUAL_VIDEO_LENGTH - imageStartTimes[i];  // ✅ 동적!
