...
// | v19.9 (1월9일 문서) | 자막 정상 표시됨 |
// 
// 🔴 결정: 더 넓은 Width = 더 안전!
// - Width: 1000px (위키 v19.4)
// - Offset Y: -0.23 (위키 v19.4)
// - lineHeight: 1.8 (v19.5 - 가독성)
// - Opacity: 0.7 (v19.5 - 가독성)
// ============================================================
...
// ============================================================
// 🔴 v19.x 종합 자막 규칙 (v19.4 + v19.5 최적 조합!)
// ============================================================
const SUBTITLE_FINAL_RULES = {
    font: {
        family: "Noto Sans KR",
        size: 60,                  // 5060세대!
        color: "#ffffff",
    ...
    style: {
        lineHeight: 1.8,           // v19.5 (가독성 우수)
        letterSpacing: 2           // 이모지 겹침 방지
    },
    background: {
        color: brandingConfig?.branding?.primaryColor || "#000000",
        opacity: 0.7              // v19.5 (가독성 우수)
    },
    size: {
        width: 1000,              // 🔴 v19.4! (더 넓음 = 안전)
        height: 400
    },
    position: {
        offsetX: 0,
...
if (hookText) {
    ...
    subtitleClips.push({
        asset: {
            ...
            style: { lineHeight: 1.8, letterSpacing: 2 },
...
for (let i = 0; i < segments.length; i++) {
    ...
    subtitleClips.push({
        asset: {
            type: "rich-text",
            text: formattedText,
            font: SUBTITLE_FINAL_RULES.font,
            style: SUBTITLE_FINAL_RULES.style,
            background: SUBTITLE_FINAL_RULES.background,
            align: { horizontal: "center", vertical: "middle" }
        },
        start: subtitleStartTimes[i],
        length: segmentTimes[i],
        position: "center",
        width: SUBTITLE_FINAL_RULES.size.width,    // 🔴 1000px!
        height: dynamicHeight,
        offset: {
            x: SUBTITLE_FINAL_RULES.position.offsetX,
            y: SUBTITLE_FINAL_RULES.position.offsetY  // 🔴 -0.23!
        }
    });
}
...
return [{
    json: {
        ...
        debug: {
            version: "20.29-V19X-COMBINED",
            note: "v19.4(Width 1000, OffsetY -0.23) + v19.5(lineHeight 1.8, Opacity 0.7)",
            v19CombinedRules: {
                width: SUBTITLE_FINAL_RULES.size.width,
                offsetY: SUBTITLE_FINAL_RULES.position.offsetY,
                lineHeight: SUBTITLE_FINAL_RULES.style.lineHeight,
                backgroundOpacity: SUBTITLE_FINAL_RULES.background.opacity,
                fontSize: SUBTITLE_FINAL_RULES.font.size,
                letterSpacing: SUBTITLE_FINAL_RULES.style.letterSpacing
            },
            sources: {
                v19_4_wiki: "Width 1000px, OffsetY -0.23",
                v19_5_finalConfig: "lineHeight 1.8, Opacity 0.7",
                v19_9_mentioned: "자막이 정상 작동했던 버전"
            },
            timing: {
