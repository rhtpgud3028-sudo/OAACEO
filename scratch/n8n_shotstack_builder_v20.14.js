...
// v20.14 FINAL-ADJUSTED (2026-01-20)
// ============================================================
// 🔥 v20.14 핵심 변경:
// 1. BGM 볼륨 0.1 (TTS 강조하면서 BGM 유지)
// 2. 동적 싱크: 마지막 슬라이드 65초까지 연장! (v20.12 복원)
// 3. 폴백 모드만 균등 배분
// 4. 디버그 정보 강화
// ============================================================
...
// 🆕 v20.14: BGM 적절히 유지! TTS 강조!
const TTS_VOLUME = 1.0;
const BGM_VOLUME = 0.1;  // 0.15 → 0.1 (TTS 강조하면서 BGM 유지)
...
// 🆕 v20.13: 최소 12개 segments 보장!
if (segments.length < TARGET_SEGMENTS) {
    parseErrors.push(`⚠️ segments.length=${segments.length}, expected=${TARGET_SEGMENTS}`);
}
...
// ==================== 🆕 v20.13: 균등 시간 배분 (12 슬라이드 보장!) ====================
let segmentTimes = [];
let subtitleStartTimes = [];
let imageStartTimes = [];

// 🆕 v20.13: 각 슬라이드 정확히 5초씩! (60초 / 12 = 5초)
...
if (USE_DYNAMIC_SYNC && ttsAlignment) {
    ...
    for (let i = 0; i < segments.length; i++) {
        ...
        // 🆕 v20.14: 마지막 슬라이드는 65초까지 연장! (v20.12 복원)
        if (i === segments.length - 1) {
            segmentTimes.push(TARGET_TOTAL - startTime);  // 65초까지!
        } else {
    ...
    if (segmentTimes.length > 0) {
        ...
        segmentTimes[lastIndex] = Math.min(remaining, 7);  // 최대 7초!
    }
}
...
// 🆕 v20.13: 이미지 슬라이드 균등 배분!
for (let i = 0; i < segments.length; i++) {
    // 이미지가 충분하면 순서대로 사용, 부족하면 순환
    const imageIndex = i % Math.max(1, images.length);
    const src = images[imageIndex] || images[0] || placeholderImage;

    // 🆕 v20.13: 모든 슬라이드 동일한 길이! (마지막도 최대 7초)
    const slideLength = segmentTimes[i] + 0.5;
